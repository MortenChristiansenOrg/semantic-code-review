...
test('partial feedback batches report invalid items, keep valid items atomic, and retry idempotently', (t) => {
  ...
  assert.equal(repo.readJson(repo.feedbackPath('manifest.json')).threads.length, 2);
  ...
  assert.equal(repo.readJson(repo.feedbackPath('threads/valid-2.json')).comments.length, 1);
  repo.feedback('thread', 'reply-batch', '--partial', '--replies', JSON.stringify(replies));
  assert.equal(repo.readJson(repo.feedbackPath('threads/thread-0.json')).comments.length, 2);
  const invalid = repo.readJson(repo.feedbackPath('threads/valid-2.json')); invalid.comments[0].body = '';
  repo.write(repo.feedbackPath('threads/valid-2.json'), JSON.stringify(invalid));
...
test('viewer snapshot reads unchanged documents only on fallback and sees external changes/deletion', (t) => {
  ...
  repo.remove(repo.feedbackPath()); assert.equal(snapshot().revision, initial.revision);
});
...
test('viewer sends a failed batch once and deterministic retries do not duplicate feedback', (t) => {
  ...
  assert.equal(repo.readJson(repo.feedbackPath('manifest.json')).threads.length, 1);
  ...
  assert.equal(repo.readJson(repo.feedbackPath(`threads/${first.exported[0].threadId}.json`)).comments.length, 2);
});
