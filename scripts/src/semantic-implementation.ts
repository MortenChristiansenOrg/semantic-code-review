...
function stackSummary(artifact, root) {
  const binding = preparedBranchBinding(root, artifact);
...
function validateStack(paths, options) {
  ...
  const result = stackSummary(artifact, paths.root);
  if (json) {
  ...
  if (result.cumulativeBranch) {
    console.log(
      `Bound cumulative branch: ${result.cumulativeBranch} (${result.preparedHeadRevision})`,
    );
  }
}
...
export function implementationWorkflow(root: string, mode: "validate" | "prepare" | "archive", options) {
  ...
  const summary = {
    ...
    ...stackSummary(artifact, root),
    workingStages: [...artifact.workStages.keys()],
  };
  if (mode === "prepare") {
    const branch = option(options, "branch");
    if (branch) {
      prepareBranch(paths, new Map([["branch", [branch]]]), artifact, flag(options, "json"));
      Object.assign(summary, stackSummary(artifact, root));
    } else {
